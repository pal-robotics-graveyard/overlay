 ...
 * Date: 17 April 2009
 * SVN info: $Id: gazebo_ros_diffdrive.cpp 396 2011-06-14 10:57:38Z luca $
 */
...
public:
  ...
  DiffDrive() {
	...
	/// NOTE: the following two lines have been modified by R.T. in order to allow odom be a child of base_footprint
        /// avoiding by hence the conflict with many parent of one child
        tf::Transform base_footprint_to_odom(qt, vt);
//        tf::Transform base_odom_to_footprint = base_footprint_to_odom.inverse();

//        transform_broadcaster_.sendTransform(tf::StampedTransform(base_odom_to_footprint,ros::Time::now(),"base_footprint","odom"));
        ...
        odom.pose.pose.orientation  = tf::createQuaternionMsgFromRollPitchYaw( this->posIface->data->pose.roll,
                                                                               this->posIface->data->pose.pitch,
                                                                               this->posIface->data->pose.yaw);
