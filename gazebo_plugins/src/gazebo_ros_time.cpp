 ...
 * Date: 1 June 2008
 * SVN info: $Id: gazebo_ros_time.cpp 329 2011-05-17 09:39:57Z ricardo $
 */
...
#include <gazebo_plugins/gazebo_ros_time.h>
...
void GazeboRosTime::LoadChild(XMLConfigNode *node)
{
  ...
  // for rostime
  this->pub_ = this->rosnode_->advertise<roslib::Clock>("clock",10);
...
{
    // pass time to robot
    Time currentTime = Simulator::Instance()->GetSimTime();
    ...
    /***************************************************************/
    this->lock.lock();
    timeMsg.clock.fromSec(currentTime.Double());
